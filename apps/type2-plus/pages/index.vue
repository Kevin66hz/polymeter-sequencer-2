...
// Bundled under /patterns/index.json. Two entry points:
//   • top-bar PresetMenu  -> kit (whole-kit preset across 16 tracks)
//   • per-cell ▾ button   -> line (score for one track only, role-matched)
const {
  index: presetIndex,
...
// Beat-repeat rate cycle: 1/2 → 1/4 → 1/8 → 1/16 → loop. Dropped 1/32
// because a half-step repeat would collapse to 1 step (= 1/16) on the
// 16-step grid anyway; added 1/2 for longer stutter windows.
...
// MIDI device names can run long (e.g. "IAC Driver Bus 1 (Elektron Digitakt)").
// Head-truncate in both the closed <select> label and the dropdown list.
// Keep the real name accessible via the native option title / select's
// :title so hovering still shows the full string.
...
// This is how the master and kit commit paths share one button.
...
<template>
  <div class="h-screen bg-[#0a0a0a] text-[#ccc] font-mono flex flex-col overflow-hidden">
      ...
      <div class="flex-1 flex flex-col min-w-0">
        ...
        <div class="flex items-center gap-2 px-3 py-[6px] border-b border-[#161616] flex-shrink-0 flex-wrap">
          ...
          <div class="ml-auto flex items-center gap-2 flex-shrink-0">
            ...
            <!-- INT / SYNC toggle — moved here from row 2 so the sync
                 state sits right next to the BPM it drives. Only shows
                 when MIDI IN is selected (clock needs a source). -->
            ...
            <!-- BPM ボタン + オーバーレイ. When SYNC is engaged, the value
                 shown is already the externally-clocked BPM (watch in
                 useSequencerStore mirrors midiIn.state.syncBpm → bpm).
                 We colorize green to signal "this is coming from outside"
                 so the user isn't surprised that their knob edits get
                 overwritten on the next pulse. -->
            ...
            <!-- REC — toggle. Write-path gated on (recording && playing)
                 in the store; clicking while stopped is allowed (armed). -->
            <button
              class="border w-[26px] h-[24px] flex items-center justify-center text-[13px] rounded-sm flex-shrink-0 leading-none"
              :class="recording
        ...
        <div class="flex items-center gap-3 px-3 py-[5px] flex-wrap flex-shrink-0">
          ...
          <div class="flex gap-0.5 rounded-sm overflow-hidden border border-[#222]">
            ...
            <button class="text-[9px] px-2 py-1"
              :class="viewMode==='star' ? 'bg-[#2a2a2a] text-[#ddd]' : 'bg-transparent text-[#555] hover:text-[#999]'"
              @click="viewMode='star'">✦ STAR</button>
            <button class="text-[9px] px-2 py-1"
          ...
          <!-- MAPPING toggle — only visible with MIDI IN selected, since
               learn needs a MIDI source. Count badge surfaces the live
               mapping total so the user isn't guessing before SAVE. -->
          ...
          <!-- REP + rate — note repeat sits just left of ALL MUTE -->
    ...
    <div class="flex-1 flex min-h-0 overflow-hidden">
      ...
      <!-- ── STAR VIEW (Phase 5 Stage 3a-3c: Three.js 3D star system) ── -->
      <template v-else-if="viewMode==='star'">
      ...
      <!-- ── SOLAR VIEW (Stage 3-Solar: per-track elliptical orbits) ── -->
      <template v-else-if="viewMode==='solar'">
        <div class="flex-1 min-h-0 relative" @click="onBackgroundClick">
          <ClientOnly>
            <SolarView
              :tracks="tracks"
